...
type Client interface {
	...
	// Returns a Swarm for the specified infohash, creating one if it doesn't
	// exist.
	Swarm(infohash BTID)
}

type Swarm interface {
	// The infohash of the torrent this swarm exists for
	Id() BTID
	// Client() Client
...
func (s *swarm) GetInfo() (info *bencoding.Dict) {
	...
	if err != nil {
		logger.Fatalf("unable to listen for peer connections: %v", err)
	}

	// Handles a connection to a peer, either incoming or outgoing
	handleConnection := func(conn *net.TCPConn, peer *RemotePeer) {
		wg.Add(1)
		go func() {
			defer wg.Done()
						...
						case msgExtended:
							...
							if dataM, hasM := data.(bencoding.Dict)["m"]; hasM {
								if mdxId, hasMdx := dataM.(bencoding.Dict)["ut_metadata"]; hasMdx {
									logger.Printf("Peer %v supports metadata exchange, using extension ID %v.", peer, mdxId)
								} else {
									logger.Printf("Peer %v does not support metadata exchange!", peer)
									return
								}
							} else {
								logger.Printf("Peer %v does not support any extensions!", peer)
								return
							}
